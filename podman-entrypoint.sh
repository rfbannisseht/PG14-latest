...
##	chmod 700 "$PGDATA"
	...
	#mkdir -p  ${REPMGR_HOME} && chown -R repmgr: ${REPMGR_HOME}  && chmod 755 ${REPMGR_HOME}
...
psql -h localhost -U postgres -c "create extension if not exists pgagent;"
psql -h localhost -U postgres -c "CREATE LANGUAGE plpgsql;"
